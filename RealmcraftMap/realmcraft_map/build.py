...
TILE = 256
CACHE_VERSION = 'v2-columns-yxz'


def read_surface(path, cache, cached_only=False):
    before = path.stat()
    ...
    if cached.exists():
        try:
            with np.load(cached, allow_pickle=False) as a:
                return a['ids'], a['heights'], str(a['sha256']), a['columns'].tobytes(), True
        except (ValueError, OSError, KeyError, zipfile.BadZipFile, EOFError):
            pass
    if cached_only:
        raise ChunkError('PENDING_CACHE')
    ...
    buffer = io.BytesIO()
    np.savez_compressed(buffer, ids=ids, heights=heights, sha256=np.array(digest), columns=np.frombuffer(columns,dtype=np.uint8))
    ...
    return ids, heights, digest, columns, False
...
def build(source, output, *, workers=16, radius=None, cache=None, cached_only=False):
    ...
    manifest = {'format': 2, 'title': source.name, 'generatedAt': datetime.now(timezone.utc).isoformat(),
                'tileSize': TILE, 'registry': NAMES, 'palette': PALETTE.tolist(), 'dimensions': {}, 'errors': [], 'pending': [],
                'scope': 'partial' if cached_only else ('complete' if radius is None else f'radius:{radius}'), 'sourceFileCount': len(files)}
    audit = []
    ...
    try:
        for prefix, label in [('o', 'Oberwelt'), ('n', 'Nether')]:
            ...
            def read(path):
                try:
                    return path, read_surface(path, cache, cached_only), None
            ...
            with ThreadPoolExecutor(max_workers=workers) as executor:
                for i, (path, result, error) in enumerate(executor.map(read, selected), 1):
                    if error == 'PENDING_CACHE':
                        manifest['pending'].append(path.name)
                    elif error:
                        manifest['errors'].append({'file': path.name, 'reason': error})
                    else:
                        top, h, digest, columns, cached = result
            ...
            manifest['dimensions'][prefix] = {'label': label, 'bounds':[xmin,zmin,xmax,zmax],
                'chunks':chunks, 'count':len(chunks), 'grids':grids, 'regions':region_keys,
                'fitBounds':[min(int(k.split(',')[0]) for k in chunks),min(int(k.split(',')[1]) for k in chunks),max(int(k.split(',')[0]) for k in chunks)+16,max(int(k.split(',')[1]) for k in chunks)+16],
        ...
        if not manifest['dimensions'] and cached_only:
            raise ValueError('No cached chunks available. Run a regular import first.')
        ...
        for path in (Path(__file__).parent/'web').iterdir():
            if path.is_file(): (stage/path.name).write_bytes(path.read_bytes())
        ...
        (stage/'audit.json').write_text(json.dumps({'source':str(source), 'generatedAt':manifest['generatedAt'],
            'files':audit, 'errors':manifest['errors'], 'pending':manifest['pending']}, indent=2)+'\n')
