...
struct Savegame: Codable, Identifiable, Hashable {
    ...
    var bytes: Int64
    var source: String
    var annotationScope: String { UserDefaults.standard.string(forKey: "annotations.scope." + id) ?? world }
}
...
final class Library: @unchecked Sendable {
    ...
    func ensureObject(from source: URL, hash: String) throws -> URL {
        let object = objectURL(hash)
        let parent = object.deletingLastPathComponent()
        try fm.createDirectory(at: parent, withIntermediateDirectories: true)
        if fm.fileExists(atPath: object.path) {
            guard try hashFile(object) == hash else {
                throw LibraryError("Deduplicated storage object is damaged. Backup was not changed / Dedupliziertes Speicherobjekt ist beschädigt. Sicherung wurde nicht geändert.")
            }
            return object
        }
        let temporary = parent.appendingPathComponent("." + UUID().uuidString)
        defer { try? fm.removeItem(at: temporary) }
        try fm.copyItem(at: source, to: temporary)
        guard try hashFile(temporary) == hash else { throw LibraryError("Storage object verification failed / Speicherobjekt konnte nicht geprüft werden.") }
        try fm.setAttributes([.posixPermissions: 0o444], ofItemAtPath: temporary.path)
        try fm.moveItem(at: temporary, to: object)
        return object
    }
    func deduplicateWorld(_ worldURL: URL, manifest: [String:String]) throws {
        for (index, item) in manifest.sorted(by: { $0.key < $1.key }).enumerated() {
            if index % 1000 == 0 { progress("Speicher optimieren · \(index) …") }
            let file = worldURL.appendingPathComponent(item.key)
            let values = try file.resourceValues(forKeys: [.isRegularFileKey, .isSymbolicLinkKey])
            guard values.isRegularFile == true, values.isSymbolicLink != true else { throw LibraryError("Invalid save file during deduplication / Ungültige Datei bei Speicheroptimierung.") }
            let object = try ensureObject(from: file, hash: item.value)
            guard try hashFile(object) == item.value else { throw LibraryError("Storage object changed unexpectedly / Speicherobjekt wurde unerwartet geändert.") }
            let temporary = file.deletingLastPathComponent().appendingPathComponent("." + UUID().uuidString)
            try fm.linkItem(at: object, to: temporary)
            do {
        ...
        try assertSame(manifest, localManifest(worldURL))
    }
    func materializeWorld(_ worldURL: URL) throws {
        guard let enumerator = fm.enumerator(atPath: worldURL.path) else { throw LibraryError("Savegame-Ordner konnte nicht gelesen werden.") }
        for case let name as String in enumerator {
            let file = worldURL.appendingPathComponent(name)
            let values = try file.resourceValues(forKeys: [.isRegularFileKey, .isSymbolicLinkKey, .contentModificationDateKey])
            guard values.isSymbolicLink != true else { throw LibraryError("Verknüpfungen sind in Savegames nicht erlaubt.") }
            guard values.isRegularFile == true else { continue }
            var info = stat()
            guard lstat(file.path, &info) == 0 else { throw NSError(domain: NSPOSIXErrorDomain, code: Int(errno)) }
            guard info.st_nlink > 1 || (info.st_mode & S_IWUSR) == 0 else { continue }
            let data = try Data(contentsOf: file, options: .mappedIfSafe)
            let temporary = file.deletingLastPathComponent().appendingPathComponent("." + UUID().uuidString)
            try data.write(to: temporary, options: .atomic)
            try fm.setAttributes([.posixPermissions: 0o644], ofItemAtPath: temporary.path)
    ...
    func finish(_ staging: URL, world: String, title: String, source: String, manifest: [String:String], date: Date = Date()) throws -> Savegame {
        ...
        try deduplicateWorld(staging.appendingPathComponent(world), manifest: manifest)
        ...
        let previous = (try? entries())?.filter { $0.id != save.id && $0.world == save.world }.sorted { $0.date > $1.date }.first
        UserDefaults.standard.set("save:" + save.id, forKey: "annotations.scope." + save.id)
        if let previous { UserDefaults.standard.set(previous.id, forKey: "annotations.pending." + save.id) }
        return save
    }
    ...
    func exportLibraryArchive(to destination: URL) throws {
        progress("Library vor ZIP-Export prüfen …")
        for save in try entries() { _ = try verify(save) }
        let temporary = destination.deletingLastPathComponent().appendingPathComponent(".\(UUID().uuidString).zip")
        defer { try? fm.removeItem(at: temporary) }
        progress("Library-ZIP erstellen …")
        _ = try checked("/usr/bin/ditto", ["-c", "-k", "--norsrc", root.path, temporary.path])
        _ = try checked("/usr/bin/unzip", ["-tq", temporary.path])
        if fm.fileExists(atPath: destination.path) { _ = try fm.replaceItemAt(destination, withItemAt: temporary) }
        else { try fm.moveItem(at: temporary, to: destination) }
    }
    func compactLibrary() throws -> String {
        var saves = 0
        for save in try entries() {
            let manifest = try verify(save)
            try deduplicateWorld(worldFolder(save), manifest: manifest)
            _ = try verify(save)
            saves += 1
        }
        pruneObjects()
        return "Speicher optimiert · \(saves) Spielstände geprüft."
    }
    func pruneObjects() {
        let objects = root.appendingPathComponent(".objects", isDirectory: true)
        guard let enumerator = fm.enumerator(at: objects, includingPropertiesForKeys: [.isRegularFileKey]) else { return }
        for case let file as URL in enumerator {
            guard (try? file.resourceValues(forKeys: [.isRegularFileKey]).isRegularFile) == true else { continue }
            var info = stat()
            if lstat(file.path, &info) == 0, info.st_nlink <= 1 { try? fm.removeItem(at: file) }
        }
    }
}

// Copy only local annotations. Never change game files or the source snapshot.
enum AnnotationTransfer {
    static let prefixes = ["atlasPOI.", "atlasMarkers.", "conversation.ownedChests."]
    static func copy(from source: String, to target: String, defaults: UserDefaults) {
        guard source != target else { return }
        for prefix in prefixes {
            if let value = defaults.object(forKey: prefix + source), defaults.object(forKey: prefix + target) == nil { defaults.set(value, forKey: prefix + target) }
        }
        for suffix in [".hidden", ".visible", ".suspected"] {
            if let value = defaults.object(forKey: "chests.visibility." + source + suffix), defaults.object(forKey: "chests.visibility." + target + suffix) == nil { defaults.set(value, forKey: "chests.visibility." + target + suffix) }
        }
    }
}
