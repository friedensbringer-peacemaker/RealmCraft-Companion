...
private struct CompiledTranslation {
    let regex: NSRegularExpression
    let en: String
}
private let englishBundle: Bundle? = Bundle.main.path(forResource: "en", ofType: "lproj").flatMap { Bundle(path: $0) }
private let translationPatterns: [CompiledTranslation] = {
    guard let url = Bundle.main.url(forResource: "patterns", withExtension: "json"),
          let data = try? Data(contentsOf: url) else { return [] }
    let patterns = (try? JSONDecoder().decode([TranslationPattern].self, from: data)) ?? []
    return patterns.compactMap { item in
        ...
        return CompiledTranslation(regex: regex, en: item.en)
    }
}()
func tr(_ source: String) -> String {
    ...
    for item in translationPatterns {
        guard let match = item.regex.firstMatch(in: source, range: NSRange(source.startIndex..., in: source)) else { continue }
        var result = item.en
