    ...
    private var sidebar: some View {
        VStack(alignment: .leading, spacing: 12) {
            Picker(english ? "Topic" : "Thema", selection: $category) {
                ForEach(["all", "basics", "processing", "farms", "transport"], id: \.self) { Text(buildCategory($0, english)).tag($0) }
            }.labelsHidden().padding(.horizontal, 16).padding(.top, 16)
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    ForEach(["basics", "processing", "farms", "transport"], id: \.self) { topic in
                                ...
                                ForEach(items) { guide in
                                    Button { selected = guide.id } label: {
                                        VStack(alignment: .leading, spacing: 6) {
                                            ...
                                            Text(guide.summary.value(english)).font(.caption).foregroundStyle(.secondary).fixedSize(horizontal: false, vertical: true)
...
struct BuildGuideDetail: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    Text(guide.title.value(english)).font(.system(size: 27, weight: .bold))
            ...
            }.padding(CompanionLayout.pageInset).frame(maxWidth: 1100).frame(maxWidth: .infinity)
            ...
            LazyVGrid(columns: [GridItem(.adaptive(minimum: 180), alignment: .leading)], alignment: .leading, spacing: 10) {
                ForEach(Array(Set(plane.cells.flatMap { $0 })).sorted(), id: \.self) { key in
                    if let block = blocks[key] {
                        HStack(spacing: 8) {
                            Text(block.symbol).font(.system(size: 12, weight: .bold, design: .monospaced)).foregroundStyle(Color.black)
                                .frame(width: 34, height: 26).background(block.color == "air" ? Color(red: 0.97, green: 0.97, blue: 0.92) : buildColor(block.color))
    ...
    private var instructionBook: some View {
        ...
        return VStack(alignment: .leading, spacing: 16) {
            HStack {
                Text(english ? "2 · Build step by step" : "2 · Schritt für Schritt bauen").font(.title2.bold())
                Spacer()
            ...
            Text(english ? "Color = new in this step · pale = already present. Growth, flowing water and test positions show expected behavior, not extra materials. Each view is a labeled layer or section." : "Farbig = neu in diesem Schritt · blass = bereits vorhanden. Wachstum, fließendes Wasser und Teststellungen zeigen erwartetes Verhalten, keine zusätzlichen Materialien. Jede Ansicht zeigt eine beschriftete Ebene oder einen Schnitt.")
                .font(.caption).foregroundStyle(.secondary)
                ...
                ForEach(plane.columns.indices, id: \.self) { col in
                    ...
                    if let block = blocks[key] {
                        ...
                        Button { select(key, coordinate) } label: {
                            Text(block.symbol).font(.system(size: cellSize * 0.28, weight: .bold, design: .monospaced))
                                .foregroundStyle(Color.black.opacity(key == "." ? 0.28 : 0.9))
                                .frame(width: cellSize - 6, height: cellSize - 6)
                                .background(buildColor(block.color).opacity(0.87))
...
struct VideoTipsView: View {
    ...
    private var english: Bool { language == "en" }
    private var filtered: [VideoTip] { (tips ?? []).filter {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    HStack {
                        Text(english ? "\(filtered.count) of \(tips?.count ?? 0) videos" : "\(filtered.count) von \(tips?.count ?? 0) Videos")
                            .font(.caption).foregroundStyle(.secondary)
                        Spacer()
                        if !query.isEmpty || category != "all" || coverage != "all" {
                            Button(english ? "Reset" : "Zurücksetzen") { query = ""; category = "all"; coverage = "all" }
                            ...
                            ForEach(filtered) { tip in
                                Button { selected = tip.id } label: {
                                    VStack(alignment: .leading, spacing: 7) {
                                        Text(tip.title.value(english)).font(.headline)
                                        ...
                                        .background(visible?.id == tip.id ? theme.accent.opacity(0.15) : theme.surface)
                                        .overlay(alignment: .leading) { Rectangle().fill(visible?.id == tip.id ? theme.accent : .clear).frame(width: 3) }
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                if !query.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty && !tip.steps.isEmpty {
                    ...
                    }.padding(18).frame(maxWidth: .infinity, alignment: .leading).companionPanel()
                }
            ...
            }.padding(CompanionLayout.pageInset).frame(maxWidth: 1000).frame(maxWidth: .infinity)
