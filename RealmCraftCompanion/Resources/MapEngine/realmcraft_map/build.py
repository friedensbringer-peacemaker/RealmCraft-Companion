...
TILE = 256
CACHE_VERSION = 'v5-columns-biomes-chests-signs'
...
def read_surface(path, cache):
    ...
    if cached.exists():
        try:
            with np.load(cached, allow_pickle=False) as a:
                return a['ids'], a['heights'], str(a['sha256']), a['columns'].tobytes(), a['biomes'].tobytes(), json.loads(str(a['chests'])), json.loads(str(a['signs'])), True
    ...
    sign_records, sign_errors = scan_signs(data, path.name, chunk)
    sign_data = {'records': sign_records, 'errors': sign_errors}
    np.savez_compressed(cached, ids=ids, heights=heights, sha256=np.array(digest), columns=np.frombuffer(columns,dtype=np.uint8), biomes=np.frombuffer(chunk.biomes,dtype=np.uint8), chests=np.array(json.dumps(chest_data)), signs=np.array(json.dumps(sign_data)))
    return ids, heights, digest, columns, chunk.biomes, chest_data, sign_data, False
...
def build(source, output, *, workers=16, radius=None, cache=None):
    ...
    manifest = {'format': 2, 'title': source.name, 'generatedAt': datetime.now(timezone.utc).isoformat(),
                'tileSize': TILE, 'itemNames': json.loads((Path(__file__).parent/'item_names.json').read_text()), 'chestErrors': [], 'signErrors': [], 'signSchema': 1, 'biomeNames': json.loads((Path(__file__).parent/'biomes.json').read_text()), 'registry': NAMES, 'palette': PALETTE.tolist(), 'dimensions': {}, 'errors': [],
    ...
    try:
            ...
            with ThreadPoolExecutor(max_workers=workers) as executor:
                for i, (path, result, error) in enumerate(executor.map(read, selected), 1):
                    if error:
                        ...
                        top, h, digest, columns, biome_data, chest_data, sign_data, cached = result
        ...
        (stage/'audit.json').write_text(json.dumps({'source':str(source), 'generatedAt':manifest['generatedAt'],
            'files':audit, 'errors':manifest['errors'], 'chestErrors':manifest['chestErrors'], 'signErrors':manifest['signErrors']}, indent=2)+'\n')
