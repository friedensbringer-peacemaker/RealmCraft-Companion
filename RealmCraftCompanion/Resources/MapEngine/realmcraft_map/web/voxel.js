...
class AtlasVoxelViewer {
 constructor(data,layers){
  this.data=data;this.layers=layers;this.keys=new Set();this.n=64;this.h=256;this.ticket=0;
  ...
  this.status=el('p','','voxel-status');this.status.setAttribute('role','status');this.canvas=el('canvas','','voxel-canvas');this.canvas.tabIndex=0;this.canvas.setAttribute('aria-label',t('3D-Welt. Ziehen zum Umschauen. WASD oder Pfeiltasten zum Gehen; im Flugmodus Q und E für die Höhe. Escape zum Schließen.','3D world. Drag to look around. WASD or arrows to walk; Q and E for altitude in flight mode. Escape to close.'));
  this.hint=el('p',t('Vereinfachte Blöcke · 64 × 64 Blöcke · Wasser und Glas sind undurchsichtig · Keine Spielfiguren','Simplified cubes · 64 × 64 blocks · Water and glass are opaque · No characters'),'voxel-hint');
  this.controls=el('div','','voxel-controls');
  for(const [key,label]of [['w',t('↑ Vorwärts','↑ Forward')],['s',t('↓ Zurück','↓ Back')],['a',t('← Links','← Left')],['d',t('Rechts →','Right →')],['j','↶'],['l','↷'],['i',t('Blick ↑','Look ↑')],['k',t('Blick ↓','Look ↓')]]){
  ...
  close.onclick=()=>this.close();this.orbit.onclick=()=>this.setMode('orbit');this.eye.onclick=()=>this.setMode('walk');this.fly.onclick=()=>this.setMode('fly');this.recenter.onclick=()=>this.open({x:this.originX+this.camera[0],z:this.originZ+this.camera[2],eyeY:this.camera[1],dimension:this.dimension,mode:this.mode});
  ...
  this.canvas.addEventListener('wheel',e=>{e.preventDefault();if(this.mode==='orbit')this.distance=Math.max(8,Math.min(300,this.distance*Math.exp(e.deltaY*.002)));},{passive:false});
  const keyFor=e=>({ArrowUp:'w',ArrowDown:'s',ArrowLeft:'a',ArrowRight:'d'})[e.key]||e.key.toLowerCase();
  this.panel.addEventListener('keydown',e=>{e.stopPropagation();if(e.key==='Escape'){this.close();return;}const key=keyFor(e);if(['w','a','s','d','q','e','shift','i','j','k','l'].includes(key)){e.preventDefault();if(this.mode==='orbit')this.setMode('walk');this.keys.add(key);if(!e.repeat)this.nudge(key);}});
 ...
 open(point){
  ...
  this.originX=Math.floor(point.x)-32;this.originZ=Math.floor(point.z)-32;this.camera=[32,80,32];this.ready=false;this.count=0;this.keys.clear();this.ticket++;
  ...
  this.setMode(point.mode||'orbit');this.canvas.focus();this.last=performance.now();cancelAnimationFrame(this.frame);this.tick(this.last);const ticket=this.ticket;this.load(ticket).catch(()=>{if(ticket===this.ticket){this.panel.setAttribute('aria-busy','false');this.status.textContent=t('Dieser Ausschnitt konnte nicht aufgebaut werden. Zur Karte zurückkehren und einen anderen Ort wählen.','Could not build this area. Return to the map and choose another location.');}});
 }
 async load(ticket){
  const entries=[];for(let rz=Math.floor(this.originZ/256);rz<=Math.floor((this.originZ+63)/256);rz++)for(let rx=Math.floor(this.originX/256);rx<=Math.floor((this.originX+63)/256);rx++)if(this.layers.has(this.dimension,rx,rz))entries.push(this.layers.ensure(this.dimension,rx,rz));
  ...
  this.blocks=blocks;const mesh=AtlasVoxelCore.mesh(blocks,n,this.h,this.data.palette);this.gl.bindBuffer(this.gl.ARRAY_BUFFER,this.buffer);this.gl.bufferData(this.gl.ARRAY_BUFFER,mesh,this.gl.STATIC_DRAW);this.count=mesh.length/6;
  let y=255;while(y>0&&AtlasVoxelCore.air(blocks[(y*n+32)*n+32]))y--;
  this.ground=Number.isFinite(this.point.eyeY)?this.point.eyeY-1.7:Number.isFinite(this.point.y)?this.point.y+1:y+1;this.camera=[32.5,this.ground+1.7,32.5];this.target=[32.5,this.ground,32.5];this.ready=true;this.panel.setAttribute('aria-busy','false');
 ...
 nudge(key){
  ...
  if(this.mode==='walk')this.camera=AtlasVoxelCore.walk(this.blocks,this.n,this.h,this.camera,dx,dz);
  else if(this.mode==='fly'){this.camera[0]+=dx;this.camera[2]+=dz;this.camera[1]+=.3*(Number(key==='e')-Number(key==='q'));}
 }
 setMode(mode){this.mode=mode;this.yaw=Math.PI;this.pitch=mode==='orbit'?.6:0;this.distance=80;this.keys.clear();this.orbit.setAttribute('aria-pressed',String(mode==='orbit'));this.eye.setAttribute('aria-pressed',String(mode==='walk'));this.fly.setAttribute('aria-pressed',String(mode==='fly'));this.controls.hidden=mode==='orbit';this.canvas.focus();}
 tick(now){
  ...
  if(this.mode==='orbit'){target=this.target;eye=target.map((v,i)=>v-dir[i]*this.distance);}else{
   const speed=dt*(this.mode==='walk'?(this.keys.has('shift')?7:4):(this.keys.has('shift')?20:7)),forward=Number(this.keys.has('w'))-Number(this.keys.has('s')),side=Number(this.keys.has('d'))-Number(this.keys.has('a'));
   ...
   this.camera[0]=Math.max(.25,Math.min(63.75,this.camera[0]));this.camera[2]=Math.max(.25,Math.min(63.75,this.camera[2]));this.camera[1]=Math.max(.2,Math.min(290,this.camera[1]));eye=this.camera;target=eye.map((v,i)=>v+dir[i]);
  }
  this.gl.uniformMatrix4fv(this.uniform,false,AtlasVoxelCore.matrix(eye,target,w/h));this.gl.drawArrays(this.gl.TRIANGLES,0,this.count);
  const status=(this.mode==='orbit'?t('Ziehen: drehen · Scrollen: Abstand','Drag: rotate · Scroll: distance'):this.mode==='walk'?t('Maus ziehen: umsehen · WASD / Pfeiltasten: gehen · Umschalt: schneller · Stufen automatisch','Drag mouse: look · WASD / arrows: walk · Shift: faster · Automatic steps'):t('Ziehen: umsehen · WASD: frei fliegen · Q/E: Höhe · Ohne Kollision','Drag: look · WASD: fly freely · Q/E: altitude · No collision'))+` · X ${Math.floor(this.originX+this.camera[0])} Y ${Math.floor(this.camera[1])} Z ${Math.floor(this.originZ+this.camera[2])} · ${this.warning||''}`;if(this.status.textContent!==status)this.status.textContent=status;
 }
 close(){this.panel.hidden=true;this.ticket++;this.keys.clear();cancelAnimationFrame(this.frame);this.returnFocus?.focus();}
}
