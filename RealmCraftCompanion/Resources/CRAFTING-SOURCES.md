...
## Illustrated walkthroughs and agent scope · 1.7.57

`CraftingWalkthrough` derives numbered steps from the same recipe quantities, station procedures and authored obtaining steps used by the browser. Original schematic drawings cover every documented entry without downloading a graphics pack; explicit mapped optional artwork replaces matching sketches when enabled. Diagrams are not game captures or new RealmCraft evidence.

Agent export supports manual selection, current personal confirmations only, or all documented guides. Optional manual-selection prerequisites include recipe variants, ingredient alternatives, obtaining references and station recipes through a visited graph; cycles do not imply an executable endless recipe chain. These extra entries are references for one batch, not aggregated resource totals. Structured JSON carries ingredient options, yields, grids, ordered steps and inclusion/evidence status; Markdown contains the same instructions and readable text grids, without image attachments.

Instruction content fingerprints retain the prior semantic recipe/source input so the new presentation does not erase existing personal confirmations. Changed source content still requires renewed confirmation. Conversation shares the walkthrough and retains a chosen material, variant, quantity and step cursor; ambiguous names require clarification. Unknown stock remains separate from required ingredients.
