(() => {
  ...
  let savedOrientation=null,orientationError=false;
  const orientation=()=>({angle:view.angle||0,flipX:!!view.flipX,flipZ:!!view.flipZ});
  try {
    const saved=JSON.parse(localStorage.getItem(orientationKey)||'null');
    if(saved && typeof saved.angle==='number' && Number.isFinite(saved.angle) && typeof saved.flipX==='boolean' && typeof saved.flipZ==='boolean'){
      savedOrientation={angle:((saved.angle%(2*Math.PI))+2*Math.PI)%(2*Math.PI),flipX:saved.flipX,flipZ:saved.flipZ};
      Object.assign(view,savedOrientation);
    }
  }catch{}
  function orientationStatus(){
    const current=orientation(),matches=savedOrientation && Math.abs(current.angle-savedOrientation.angle)<1e-10 && current.flipX===savedOrientation.flipX && current.flipZ===savedOrientation.flipZ;
    $('orientation-status').textContent=orientationError?(english?'Could not save. Browser storage is unavailable.':'Speichern fehlgeschlagen. Der Browserspeicher ist nicht verfügbar.'):matches?(english?'Saved for the next reload.':'Für das nächste Laden gespeichert.'):(english?'Not saved. Click Save orientation to keep this view.':'Nicht gespeichert. Mit „Ausrichtung speichern“ für das nächste Laden merken.');
  }
  ...
  function draw(){
    ...
    poi.draw(ctx,screen,width,height);
    if(selected && selected.dimension===dimension){const p=screen(selected.x+.5,selected.z+.5),size=Math.max(8,view.zoom);ctx.save();ctx.translate(p.x,p.y);ctx.scale(view.flipX?-1:1,view.flipZ?-1:1);ctx.rotate(view.angle);ctx.strokeStyle='#fff4b0';ctx.lineWidth=2;ctx.strokeRect(-size/2,-size/2,size,size);ctx.restore();}
  ...
  function rotate(angle){view.angle=((angle%(2*Math.PI))+2*Math.PI)%(2*Math.PI);requestDraw();}
  ...
  function changeDimension(value){dimension=value;$('biome-readout').textContent=english?'Saved biome · point at the map':'Gespeichertes Biom · auf Karte zeigen';const d=data.dimensions[dimension];$('dimension').value=dimension;$('map-label').textContent=d.label+(dimension==='n'?' · Y ≤ 90':'');$('slice-note').hidden=dimension!=='n';$('chunk-count').textContent=format(d.count);$('block-count').textContent=(d.count*256/1e6).toLocaleString('de-AT',{maximumFractionDigits:2})+' Mio.';$('selection').hidden=true;selected=null;
    ...
    const issues=[];if(data.errors.length)issues.push(`${data.errors.length} Dateien konnten nicht gelesen werden; diese Bereiche fehlen.`);if(d.unknownIds.length)issues.push(`${d.unknownIds.length} unbekannte Blocktypen sind magenta markiert.`);$('error-note').textContent=issues.join(' ');$('error-note').hidden=!issues.length;renderMarkers();poi.setDimension(dimension);fit();}
  ...
  function pick(px,py){if(poi.pick(px,py,screen)){$('selection').hidden=true;return;}const pos=worldAt(px,py,view,width,height),x=Math.floor(pos.x),z=Math.floor(pos.z),block=blockAt(x,z);if(!block){$('search-message').textContent='An dieser Stelle ist kein Chunk in der Karte gespeichert.';return;}
  ...
  $('orientation-save').onclick=()=>{
    try{const value=orientation();localStorage.setItem(orientationKey,JSON.stringify(value));savedOrientation=value;orientationError=false;}
  ...
  $('compass').onclick=e=>rotate(view.angle+(e.shiftKey?-1:1)*Math.PI/4);
  $('compass').oncontextmenu=e=>{e.preventDefault();$('mirror-reset').click();};
  const rotationHelp=english?'Rotate 45° · Shift: reverse · Right-click: north · Option/Alt + scroll: freely rotate · Q/E: rotate · R: north':'45° drehen · Umschalt: zurück · Rechtsklick: Norden · Option/Alt + Scrollen: frei drehen · Q/E: drehen · R: Norden';
  ...
  $('locate').onsubmit=e=>{e.preventDefault();const x=Number($('x-input').value),z=Number($('z-input').value);if(!Number.isSafeInteger(x)||!Number.isSafeInteger(z))return;const block=blockAt(x,z);showBiome(x,z);$('search-message').textContent=block?'': 'Kein gespeicherter Chunk an diesen Koordinaten.';view={...view,x:x+.5,z:z+.5,zoom:Math.max(view.zoom,3)};document.body.classList.remove('sidebar-open');requestDraw();};
  ...
  canvas.addEventListener('wheel',e=>{e.preventDefault();const r=canvas.getBoundingClientRect();if(e.altKey)rotate(view.angle+clamp(e.deltaY,-200,200)*.005);else view=zoomAt(e.clientX-r.left,e.clientY-r.top,Math.exp(-clamp(e.deltaY,-200,200)*.006),view,width,height);requestDraw();},{passive:false});
  ...
  const mapKey=e=>{const step=80;switch(e.key){case'ArrowLeft':view=panBy(step,0,view);break;case'ArrowRight':view=panBy(-step,0,view);break;case'ArrowUp':view=panBy(0,step,view);break;case'ArrowDown':view=panBy(0,-step,view);break;case'+':case'=':view.zoom=clamp(view.zoom*1.5,.015625,32);break;case'-':view.zoom=clamp(view.zoom/1.5,.015625,32);break;case'f':case'F':fit();break;case'q':case'Q':rotate(view.angle-Math.PI/4);break;case'e':case'E':rotate(view.angle+Math.PI/4);break;case'r':case'R':$('mirror-reset').click();break;case'PageUp':setLevel(yLevel+(e.shiftKey?10:e.altKey?5:1));break;case'PageDown':setLevel(yLevel-(e.shiftKey?10:e.altKey?5:1));break;case'Escape':$('close-selection').click();break;default:return;}e.preventDefault();requestDraw();};
  ...
  document.addEventListener('keydown',e=>{
    ...
    if(['q','e','r'].includes(e.key.toLowerCase()))mapKey(e);
  });
