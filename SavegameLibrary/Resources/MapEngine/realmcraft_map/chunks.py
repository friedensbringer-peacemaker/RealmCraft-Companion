...
class Chunk:
    ...
    block_data_end: int
    biomes: bytes = b""  # v9: 16 raw biome IDs, index (local_x & 12) | ((local_z >> 2) & 3)
...
def decode(data: bytes, filename=None) -> Chunk:
    ...
    return Chunk(x, z, dimension, result.reshape(256, 16, 16), pos, data[pos:pos+16] if len(data)>=pos+16 else b"")
