...
class ChestMapTests(unittest.TestCase):
 def test_export_and_cache_keep_inventory_and_source_unchanged(self):
  from realmcraft_map.build import build,read_surface
  with tempfile.TemporaryDirectory() as td:
   root=Path(td);source=root/'world';source.mkdir();p=source/'o.-64,0'
   data=chunk()+record(item(3157,44,2));p.write_bytes(data)
   cache=root/'cache';cache.mkdir()
   first=read_surface(p,cache);second=read_surface(p,cache)
   self.assertFalse(first[-1]);self.assertTrue(second[-1]);self.assertEqual(first[-2],second[-2])
   result=build(source,root/'map',cache=cache,workers=1)
   chest=result['dimensions']['o']['chests']['-53,68,10']
   self.assertTrue(chest['readable']);self.assertEqual(chest['items'][0]['quantity'],44)
   self.assertIn('3157',result['itemNames']);self.assertEqual(p.read_bytes(),data)
